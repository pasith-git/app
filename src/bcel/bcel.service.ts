...
@Injectable()
export class BcelService {
    private merchantId = "mch62baa8923023e";
    private tid = null;
    private shopCode = null;
    private uuid = null;
    private lastToken = new Date().getTime() - 30 * 60 * 1000 + "0000";
    private debug = false;
    private pubnub = null;
    private onPaid = null;
    private qrCode = null;
    ...
    pubnubSubscribe() {
        let channel = this.shopCode ? 'mcid-' + this.merchantId + "-" + this.shopCode : 'uuid-' + this.merchantId + "-" + this.uuid;
        if (this.tid && this.shopCode) {
            channel = 'tid-' + this.merchantId + "-" + this.shopCode + '-' + this.tid;
        }

        const pns = new PubNub({
            uuid: 'BCELBANK',
            subscribeKey: 'sub-c-91489692-fa26-11e9-be22-ea7c5aada356',
            ssl: true
          });
    ...
    getCode(params, qrCode) {
        ...
        if (params.expireTime) $field33.push(["03", this.getExpiredTime(this.expireTime)]); // expired time
    ...
    getExpiredTime(expireTime) {
        ...
        let expiredTime = dateTime.getFullYear().toString()
