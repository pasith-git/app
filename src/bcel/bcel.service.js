...
exports.BcelService = void 0;
var common_1 = require("@nestjs/common");
var pubnub_1 = require("pubnub");
var BcelService = /** @class */ (function () {
    function BcelService() {
        this.merchantId = "mch62baa8923023e";
        this.tid = null;
        this.shopCode = null;
        this.uuid = null;
    ...
    BcelService.prototype.pubnubSubscribe = function () {
        var channel = this.shopCode ? 'mcid-' + this.merchantId + "-" + this.shopCode : 'uuid-' + this.merchantId + "-" + this.uuid;
        if (this.tid && this.shopCode) {
            channel = 'tid-' + this.merchantId + "-" + this.shopCode + '-' + this.tid;
        }
        var pns = new pubnub_1["default"]({
            uuid: 'BCELBANK',
            subscribeKey: 'sub-c-91489692-fa26-11e9-be22-ea7c5aada356',
            ssl: true
        });
        pns.subscribe({
            channels: [channel],
            timetoken: parseInt(this.lastToken)
        });
    };
    BcelService.prototype.paymentCallback = function (res) {
        if (this.onPaid)
            this.onPaid(res);
        this.onPaid = null; // Stop receiving additional callback
        this.pubnub.stop();
    };
    BcelService.prototype.stop = function () {
        this.pubnub.stop();
    };
    BcelService.prototype.payCode = function (res) {
        if (this.qrCode)
            this.qrCode(res);
    };
    BcelService.prototype.getCode = function (params, qrCode) {
        if (this.debug)
            console.log("get code parameters:", params);
        this.qrCode = qrCode;
        if (params.mcc)
            this.mcc = params.mcc;
        if (params.ccy)
        ...
        var fullqr = rawqr + this.buildqr([["63", this.crc16(rawqr + "6304")]]); // Adding itself into checksum
        this.payCode(fullqr);
    };
    BcelService.prototype.getExpiredTime = function (expireTime) {
    ...
    BcelService.prototype.padLeft = function (val) {
        return val > 10 ? val : "0" + val;
    };
    BcelService.prototype.buildqr = function (arr) {
        var res = "";
        for (var i in arr) {
            var key = arr[i][0];
            var val = arr[i][1];
    ...
    BcelService.prototype.pad2 = function (data) {
        return ("0" + (data + "")).substr(-2);
    };
    BcelService.prototype.crc16 = function (s) {
        var crc = 0xFFFF;
        var j, i;
    ...
    BcelService = __decorate([
        (0, common_1.Injectable)()
    ], BcelService);
    return BcelService;
}());
exports.BcelService = BcelService;
;
