...
@Injectable()
export class MuseumIdGuard implements CanActivate {
    ...
    async canActivate(
        ...
        try {
            if (req.headers.authorization) {
                ...
                if (!user.museum_id) {
                    throw new CustomException({ error: "For first-time use, you must create museum information first (path:'/museums/first-time')", code: "FT" }, HttpStatus.FORBIDDEN);
                }
                return true;
            }
